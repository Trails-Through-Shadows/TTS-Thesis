...
Součástí hry Gloomhaven je mnoho komponent, které jsou potřeba k hraní, o čemž vypovídá i samotná váha krabice, která je přes 10 kilogramů. V následujícím výčtu jsou popsány ty nejdůležitější z nich.
...
Samotné mapy jednotlivých lokací jsou tvořeny spojováním hexagonových místností, jejichž rozložení hráčům ukazuje kniha scénářů. Místností je mnoho, proto je jejich vyhledávání mezi ostatním materiálem urychleno pomocí jedinečných identifikátorů, které jsou vždy vytištěné na dílech. Po rozvržení místností se herní plocha může ozvláštnit více pomocí samostatných hexagonových tokenů, které se pokládají na políčka, která reprezentují překážky, pasti, dveře, nebo jiné herní prvky.

\subsubsection*{Figurky}
\label{subsubsec:gh_comp_figures}

V Gloomhavenu se hráči setkávají se třemi základními typy figur, které se v rámci hry využívají. První z nich jsou figurky postav, které hráči ovládají. Každá postava má svou vlastní 3D figurku, která je unikátní a uchovaná v krabičce, aby se nepoškodila. 
...
\label{subsubsec:gh_comp_counters}
